...
export const MOUVEMENTS_OPUS55 = [
  ...
  [opus55.ARRETE + "…", `La règle de conduite, en deux phrases fixes, selon que l'agent travaille seul ou que tu suis le travail. Seul — le cas par défaut : « ${opus55.CONTINUE} » Puis « ${opus55.ARRET} » suivi des gestes qui engagent dans ce travail-là — supprimer des données, forcer un push, mettre en ligne, envoyer à quelqu'un. Si tu restes là pour répondre, la règle de continuer n'a plus lieu d'être (la documentation d'Anthropic dit de la laisser de côté) : « ${opus55.INTENTION} » Puis « ${opus55.DEMANDE} » suivi des mêmes gestes. Jamais les deux. Un autre arrêt n'entre que si tu l'as demandé.`],
  ["Les clauses du type", `Seulement celles qui s'appliquent à ce travail-là : une liste de tâches dans un fichier pour un long run, des sous-agents dont on vérifie les preuves pour un audit, les styles exclus nommément pour du design, ce qui n'a pas pu être confirmé pour une recherche, ce qui bloquerait la fusion pour une revue — et, quand le travail traverse plusieurs applications (mails, documents, tableurs, fiches), l'ordre de fouiller largement avant de rien modifier : « ${opus55.EXPLORE} »`],
...
const RAW_CHAPITRES = [
  ...
  },
  {
    ...
    points: [
      ...
      `La méthode Opus 5.5 est alignée sur la ${opus55.SOURCE.auteur}, « ${opus55.SOURCE.titre} » (${opus55.SOURCE.url}), qui fait foi. Elle est née d'un billet antérieur, « ${opus55.ORIGINE.titre} » (${opus55.ORIGINE.auteur}) : quelques consignes n'existent que là (la revue, la recherche, le long document, les trois titres de la fin), et restent tant que la documentation ne les contredit pas. Ce que la documentation dit d'abord : un bon prompt marche tel quel, et le gros du gain vient des réglages — l'effort, la consigne contre les arrêts trop tôt, les relances.`,
      "Elle contredit sur un point les méthodes Boris et crochets, et c'est voulu : elles interdisent « ne t'arrête pas » comme consigne devenue inutile, alors que la documentation dit qu'Opus 5.5, laissé seul sur un long travail, s'arrête parfois pour rendre compte, et qu'une consigne qui nomme ces arrêts aide — d'abord dans les réglages, et le message en porte la règle courte.",
    ],
  },
  {
    cle: "questions",
    ...
    points: [
      ...
      "Avec la méthode Opus 5.5 : deux questions au plus, et seulement si l'on ne peut pas deviner à quoi on verra que c'est fini, ou sur quoi l'agent travaille — et, plus rarement, si ton idée hésite entre un agent laissé seul et un travail que tu suis pour répondre. Sans indice, c'est un agent seul. Le type de travail, les consignes utiles, les styles à exclure : l'atelier les tranche sans te les demander.",
  ...
  },
  {
    ...
    points: [
      ...
      "Les modèles ne se choisissent plus : tout tourne sur Opus 5.5, avec un effort adapté à chaque rôle — moyen pour les questions et la rédaction (mesuré au banc : plus d'effort n'écrivait pas plus fidèle), élevé pour le juge. Ils sont affichés dans les réglages, pour qu'on sache ce qui tourne.",
  ...
  },
  {
    ...
    points: [
      ...
      "Rien de ce que le modèle fait déjà n'y entre. Pas de « réfléchis bien » ni d'« étape par étape » : Opus 5.5 pense avant chaque réponse, la profondeur se règle par l'effort, et la ligne ne fait que ralentir. Pas de demande de montrer son raisonnement : c'est une demande que le modèle peut refuser, et ce refus-là n'est jamais rejoué sur un autre modèle.",
      ...
      "Le message se colle tel quel dans Claude Code ou dans l'app Claude, avec Opus 5.5 choisi dans le sélecteur de modèle, en premier message d'une session neuve. Sa règle de conduite dépend de toi : laissé seul, l'agent continue sans te demander ; si tu restes là, il te dit d'abord ce qu'il va faire et te demande ton accord avant tout geste qui engage.",
      "La consigne complète contre les arrêts trop tôt n'est pas dans le message : elle se règle à côté, en consigne système posée dès le lancement de la session — jamais en cours de route, ce qui ferait perdre au modèle la réflexion déjà faite — et elle se laisse de côté quand tu suis le travail pour répondre.",
      "Quand le travail traverse plusieurs applications, le message demande à l'agent de fouiller largement avant de rien modifier, et d'agir sur ce qu'il trouve. Ne laisse donc dans ces boîtes, dossiers et fiches rien dont tu te méfies : un texte écrit par un inconnu pourrait lui dicter quoi faire.",
    ],
